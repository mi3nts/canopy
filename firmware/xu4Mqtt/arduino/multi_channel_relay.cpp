...
uint8_t Multi_Channel_Relay::scanI2CDevice(void) {
    byte error = 0, result = 0;
    uint8_t nDevices = 0;

    // Re-initialize I2C defensively before scanning.
    Wire.begin();

    DEBUG_PRINT.println("Scanning...");

    // Use the standard 7-bit scan range and leave reserved ranges untouched.
    for (uint8_t address = 0x08; address <= 0x77; address++) {
        ...
        // Keep the USB/RTOS loop responsive on boards like Nano 33 BLE.
        yield();
        delay(1);
    }
    if (nDevices == 0) {
